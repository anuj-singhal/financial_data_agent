...
class UIManager:
    ...
    def process_chat_query(self, message: str, history: List) -> str:
        ...
        try:
            ...
            if len(df) > 5:
                ...
                response += "💡 *Data loaded below. Configure chart options and click 'Generate Chart' to visualize.*"
            else:
                response += f"**Data:**\n```\n{df.to_string()}\n```"
    ...
    def chat_and_update_ui(self, message: str, history: List) -> Tuple[List, pd.DataFrame, gr.Dropdown, gr.Dropdown, gr.Dropdown, None]:
        """
        Process chat and update all UI components.
        ...
        """
        # Process the query
        response = self.process_chat_query(message, history)
        
        # Update chat history
        history = history or []
        history.append([message, response])
        
        # Get data and dropdown updates
        if self.current_df is not None and not self.current_df.empty:
            ...
            return history, *self._empty_results(), None
    ...
    def create_interface(self) -> gr.Blocks:
        ...
        with gr.Blocks(
                ...
                with gr.TabItem("📊 Query & Visualize"):
                    with gr.Row():
                        ...
                        with gr.Column(scale=1):
                            ...
                            # Chat Interface with custom setup for multiple outputs
                            chatbot = gr.Chatbot(
                                height=400,
                                label="Query Chat"
                            )
                            ...
                            gr.Examples(
                                examples=[
                                    "Show quarterly profits for all banks in 2024",
                                    "Quarterly trend analysis for ENBD",
            ...
            # Event Handlers
            # Submit button click - process query and update all components
            submit_btn.click(
                fn=self.chat_and_update_ui,
                inputs=[msg, chatbot],
                outputs=[chatbot, df_display, x_dropdown, y_dropdown, color_dropdown, plot_output]
            ).then(
                fn=lambda: "",
                inputs=[],
                outputs=[msg]
            ...
            # Clear chat button
            clear_btn.click(
                fn=lambda: ([], pd.DataFrame(), gr.update(choices=[], value=None), 
                          gr.update(choices=[], value=None), gr.update(choices=[None], value=None), None),
                inputs=[],
                outputs=[chatbot, df_display, x_dropdown, y_dropdown, color_dropdown, plot_output]
            )
